...
const SingleProduct = ({
  ...
  // Only use searchParams if not in modal mode
  const [searchParams, setSearchParams] = modalMode
    ? [new URLSearchParams(), () => {}]
    : useSearchParams();

  const itemId = !modalMode ? searchParams.get("itemId") || "" : "";
  const skuIdFromUrl = !modalMode ? searchParams.get("skuId") || "" : "";
  ...
  // Use recently viewed products hook - only if not in modal mode
  useEffect(() => {
    if (!modalMode && singleProduct) {
      const recentlyViewedFunc = useRecentlyViewed(singleProduct);
      return () => {
        // Cleanup if needed
      };
    }
  }, [modalMode, singleProduct]);
  ...
  // Custom handleAddToCart for modal mode
  const handleAddToCartWrapper = () => {
    if (modalMode && onAddToCart) {
      // Call provided onAddToCart from parent component
      const productToAdd = {
  ...
  /**
   * Update URL when active SKU changes - only if not in modal mode
   */
  useEffect(() => {
    if (modalMode) return; // Skip URL updates in modal mode
  ...
  /**
   * Auto-select SKU from URL when page loads - only if not in modal mode
   */
  useEffect(() => {
    if (modalMode) return; // Skip in modal mode
  ...
  const handlePropertySelect = (
    ...
    if (!value) {
      ...
      // Only update URL if not in modal mode
      if (!modalMode && window.history) {
  ...
  return (
    <>
      {isLoading ? (
        ...
        <div className={`${modalMode ? "bg-white" : "bg-gray-200"}`}>
          <div
            className={`w-full ${modalMode ? "max-w-full" : "max-w-[1600px]"} mx-auto px-4 py-6`}>
            ...
            {/* Explore your interests section - only show if not in modal mode */}
            {!modalMode && (
