      ...
      <div className="">
        <Swiper
          ...
          {Object.entries(filterOptions).map(([name, { icon, type }]) => (
            <SwiperSlide key={name} className="!w-auto p-1">
              <button
        ...
        {openFilter && filterOptions[openFilter].type !== "sort" && (
          <div
            ...
            <div
              ...
              onClick={(e) => e.stopPropagation()}>
              {filterOptions[openFilter].type == "sort" ? (
                // Sort dropdown
                <div className="py-2">
                  {filterOptions[openFilter].children.map((option) => (
                    <button
                      ...
                      className={`w-full text-left px-4 py-2 hover:bg-gray-100 ${
                        selectedOptions[openFilter]?.includes(option)
                ...
                <div className="p-4">
                  <div className="flex justify-between items-center mb-4">
                    <h3 className="text-lg font-semibold">{openFilter}</h3>
                    <button onClick={() => setOpenFilter("")}>
                      <X className="w-5 h-5" />
                    </button>
                  </div>
                  <div className="flex flex-wrap gap-2 mb-4">
                    {filterOptions[openFilter].children.map((child) => (
                      <button
                        key={child}
                        onClick={() => handleOptionSelect(openFilter, child)}
                        className={`px-4 py-2 rounded-full text-sm transition-colors ${
                          selectedOptions[openFilter]?.includes(child)
                  ...
                  <div className="flex justify-end gap-4 border-t pt-4">
                    <button
                      className="px-4 py-2 rounded-full border border-gray-300 text-gray-600 hover:text-gray-800"
                      onClick={() => {
                        setSelectedOptions((prev) => ({
                          ...prev,
                          [openFilter]: [],
